...
PROJECT = merge_raft
PROJECT_DESCRIPTION = merge_raft
...
erlfmt: $(ERLFMT)
	$(verbose) $(ERLFMT) --verbose --write --require-pragma --print-width=120 \
		'apps/{merge_raft,merge_raft_test}/{src,include,test}/**/*.{hrl,erl,app.src,app.src.script}' \
		'apps/{merge_raft,merge_raft_test}/{rebar.config,rebar.config.script}' \
		'apps/{merge_raft,merge_raft_test}/test/**/*.config' \
