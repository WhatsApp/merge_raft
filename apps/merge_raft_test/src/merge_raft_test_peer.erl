...
start_link(PeerNode) when is_atom(PeerNode) ->
    ...
    CookieString = binary_to_list(cookie(PeerNode)),
    Options = #{
...
-spec cookie(Node | Peer) -> Cookie when Node :: node(), Peer :: t(), Cookie :: cookie().
cookie(Node) when is_atom(Node) ->
    cookie(decode(Node));
cookie(#{
    os_pid := OsPid,
    rand_a := RandA,
    ...
    MetaULID =
        <<OsPid:1/unsigned-big-integer-unit:32, UnixTsMs:1/unsigned-big-integer-unit:48, 7:4, RandA:12/bits, 2:2,
            RandB:62/bits>>,
    Cookie = <<<<(crockford_base32_encode(C)):8>> || <<C:5>> <= MetaULID>>,
    Cookie.
...
encode(
    ...
    maybe
        ...
        Cookie = cookie(Peer),
        NodeBytes = <<Name/bytes, Cookie/bytes, $@, HostBytes/bytes>>,
