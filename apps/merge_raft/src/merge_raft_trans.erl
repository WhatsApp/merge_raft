...
child_spec(Name) ->
    merge_raft:child_spec(#{type => {registered, Name, undefined}, cb_state => Name, module => ?MODULE}).

-spec start_link(server_name()) -> gen_server:start_ret().
start_link(Name) ->
    merge_raft:start_link(#{type => {registered, Name, undefined}, cb_state => Name, module => ?MODULE}).

-spec start(server_name()) -> gen_server:start_ret().
start(Name) ->
    merge_raft:start(#{type => {registered, Name, undefined}, cb_state => Name, module => ?MODULE}).
